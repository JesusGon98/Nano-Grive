...
If you discover a security vulnerability in nano-grive, please report it by:
...
chmod 600 ~/.nano-grive/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.nano-grive/config.json` with file permissions set to `0600`
...
- Config files and API keys (`~/.nano-grive/config.json`) → **hidden** (masked by tmpfs)
...
- ✅ Run nano-grive with a dedicated user account
...
- Keep authentication data in `~/.nano-grive/whatsapp-auth` secure (mode 0700)
...
pip install --upgrade nano-grive-ai
```
...
- Subscribe to security advisories for nano-grive and its dependencies
...
1. **Isolate the Environment**
   ...
   pip install nano-grive-ai
   ```

2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash nano-grive
   sudo -u nano-grive nano-grive gateway
   ```

3. **Set Proper Permissions**
   ```bash
   chmod 700 ~/.nano-grive
   chmod 600 ~/.nano-grive/config.json
   chmod 700 ~/.nano-grive/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.nano-grive/logs/nano-grive.log
   ```
...
- **Chat history is stored locally** - protect the `~/.nano-grive` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.nano-grive/logs/nano-grive.log
   ```
...
- GitHub Security Advisories: https://github.com/JesusGon98/Nano-Grive/security/advisories
- Release Notes: https://github.com/JesusGon98/Nano-Grive/releases
