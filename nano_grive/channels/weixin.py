...
from nano_grive.config.paths import get_media_dir, get_runtime_subdir
...
class WeixinConfig(Base):
    ...
    state_dir: str = ""  # Default: ~/.nano_grive/weixin/
...
class WeixinChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                logger.error("WeChat login failed. Run 'nano_grive channels login weixin' to authenticate.")
                self._running = False
                return
    ...
    async def _send_text(
        ...
        client_id = f"nano_grive-{uuid.uuid4().hex[:12]}"
