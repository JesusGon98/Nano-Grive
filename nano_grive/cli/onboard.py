"""Interactive onboarding questionnaire for nano_grive."""
...
from nano_grive.cli.models import (
    format_token_count,
...
def _show_main_menu_header() -> None:
    ...
    console.print(
        Align.center(f"{__logo__} [bold cyan]nano_grive[{__version__}][/bold cyan]")
    )
    console.print()
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note:
        This function imports AgentDefaults from nano_grive.config.schema to get
...
def _get_channel_info() -> dict[str, tuple[str, type[BaseModel]]]:
    ...
    for name, channel_cls in discover_all().items():
        try:
            mod = importlib.import_module(f"nano_grive.channels.{name}")
