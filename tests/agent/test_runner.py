...
from nano_grive.agent.tools.base import Tool
from nano_grive.agent.tools.registry import ToolRegistry
from nano_grive.providers.base import LLMResponse, ToolCallRequest
...
def _make_loop(tmp_path):
    ...
    with patch("nano_grive.agent.loop.ContextBuilder"), \
         patch("nano_grive.agent.loop.SessionManager"), \
         patch("nano_grive.agent.loop.SubagentManager") as MockSubMgr:
...
async def test_runner_preserves_reasoning_fields_and_tool_results():
    from nano_grive.agent.runner import AgentRunSpec, AgentRunner
...
async def test_runner_calls_hooks_in_order():
    from nano_grive.agent.hook import AgentHook, AgentHookContext
    from nano_grive.agent.runner import AgentRunSpec, AgentRunner
...
async def test_runner_persists_large_tool_results_for_follow_up_calls(tmp_path):
    ...
    assert (tmp_path / ".nano_grive" / "tool-results" / "test_runner" / "call_big.txt").exists()


def test_persist_tool_result_prunes_old_session_buckets(tmp_path):
    from nano_grive.utils.helpers import maybe_persist_tool_result

    root = tmp_path / ".nano_grive" / "tool-results"
...
def test_persist_tool_result_logs_cleanup_failures(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "nano_grive.utils.helpers._cleanup_tool_result_buckets",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(OSError("busy")),
    )
    monkeypatch.setattr(
        "nano_grive.utils.helpers.logger.warning",
...
async def test_runner_uses_specific_message_after_empty_finalization_retry():
    ...
    from nano_grive.agent.runner import AgentRunSpec, AgentRunner
    from nano_grive.utils.runtime import EMPTY_FINAL_RESPONSE_MESSAGE
...
def test_snip_history_drops_orphaned_tool_results_from_trimmed_slice(monkeypatch):
    ...
    monkeypatch.setattr("nano_grive.agent.runner.estimate_prompt_tokens_chain", lambda *_args, **_kwargs: (500, None))
    token_sizes = {
        "old user": 120,
        "tool call": 120,
        "tool output": 40,
    ...
    monkeypatch.setattr(
        "nano_grive.agent.runner.estimate_message_tokens",
...
async def test_runner_keeps_going_when_tool_result_persistence_fails():
    ...
    runner = AgentRunner(provider)
    with patch("nano_grive.agent.runner.maybe_persist_tool_result", side_effect=RuntimeError("disk full")):
        result = await runner.run(AgentRunSpec(
            initial_messages=[{"role": "user", "content": "do task"}],
            tools=tools,
            model="test-model",
...
async def test_subagent_max_iterations_announces_existing_fallback(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nano_grive.agent.tools.filesystem.ListDirTool.execute", fake_execute)
...
async def test_length_recovery_gives_up_after_max_retries():
    ...
    from nano_grive.agent.runner import AgentRunSpec, AgentRunner, _MAX_LENGTH_RECOVERIES
...
async def test_backfill_missing_tool_results_inserts_error():
    ...
    from nano_grive.agent.runner import AgentRunner, _BACKFILL_CONTENT
...
async def test_backfill_noop_when_complete():
    ...
    from nano_grive.agent.runner import AgentRunner
...
async def test_microcompact_replaces_old_tool_results():
    ...
    from nano_grive.agent.runner import AgentRunner, _MICROCOMPACT_KEEP_RECENT
