...
from nano_grive.command.builtin import cmd_new, register_builtin_commands
from nano_grive.command.router import CommandContext, CommandRouter
from nano_grive.config.schema import AgentDefaults, Config
...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("nano_grive.agent.loop.SessionManager"), \
         patch("nano_grive.agent.loop.SubagentManager") as MockSubMgr, \
         patch("nano_grive.agent.loop.Dream"):
...
class TestUnifiedSessionConfig:
    ...
    def test_onboard_generated_config_contains_unified_session(self, tmp_path: Path):
        """save_config() writes 'unifiedSession' into config.json (simulates nano_grive onboard)."""
        from nano_grive.config.loader import save_config
...
class TestConsolidationUnaffectedByUnifiedSession:
    ...
    async def test_consolidation_skips_empty_session_for_unified_key(self):
        ...
        from nano_grive.agent.memory import Consolidator, MemoryStore
...
class TestStopCommandWithUnifiedSession:
    ...
    async def test_active_tasks_use_effective_key_in_unified_mode(self, tmp_path: Path):
        ...
        from nano_grive.agent.loop import UNIFIED_SESSION_KEY
    ...
    async def test_stop_command_finds_task_in_unified_mode(self, tmp_path: Path):
        ...
        from nano_grive.agent.loop import UNIFIED_SESSION_KEY
        from nano_grive.command.builtin import cmd_stop
