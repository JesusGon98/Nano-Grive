...
from nano_grive.config.schema import ChannelsConfig
from nano_grive.utils.restart import RestartNotice
...
def test_discover_plugins_loads_entry_points():
    from nano_grive.channels.registry import discover_plugins
...
async def test_manager_loads_plugin_from_dict_config():
    ...
    with patch(
        "nano_grive.channels.registry.discover_all",
...
def test_channels_login_uses_discovered_plugin_class(monkeypatch):
    from nano_grive.cli.commands import app
    ...
    monkeypatch.setattr("nano_grive.config.loader.load_config", lambda config_path=None: Config())
    monkeypatch.setattr(
        "nano_grive.channels.registry.discover_all",
...
def test_builtin_channel_default_config():
    ...
    from nano_grive.channels.telegram import TelegramChannel
...
async def test_send_with_retry_retries_on_failure():
    ...
    with patch("nano_grive.channels.manager.asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_no_retry_when_max_is_zero():
    ...
    with patch("nano_grive.channels.manager.asyncio.sleep", new_callable=AsyncMock):
        await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_send_with_retry_propagates_cancelled_error_during_sleep():
    ...
    with patch("nano_grive.channels.manager.asyncio.sleep", side_effect=cancel_during_sleep):
        with pytest.raises(asyncio.CancelledError):
            await mgr._send_with_retry(mgr.channels["failing"], msg)
...
async def test_notify_restart_done_enqueues_outbound_message():
    ...
    with patch("nano_grive.channels.manager.consume_restart_notice_from_env", return_value=notice):
