...
from nano_grive.channels.matrix import _build_matrix_text_content

import nano_grive.channels.matrix as matrix_module
from nano_grive.bus.events import OutboundMessage
from nano_grive.bus.queue import MessageBus
from nano_grive.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from nano_grive.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("nano_grive.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "nano_grive.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("nano_grive.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "nano_grive.channels.matrix.asyncio.create_task", _fake_create_task
    )
