...
from nano_grive.cli.commands import _make_provider, app
from nano_grive.config.schema import Config
from nano_grive.cron.types import CronJob, CronPayload
from nano_grive.providers.openai_codex_provider import _strip_model_prefix
from nano_grive.providers.registry import find_by_name
...
def mock_paths():
    ...
    with patch("nano_grive.config.loader.get_config_path") as mock_cp, \
         patch("nano_grive.config.loader.save_config") as mock_sc, \
         patch("nano_grive.config.loader.load_config") as mock_lc, \
         patch("nano_grive.cli.commands.get_workspace_path") as mock_ws:
...
def test_onboard_fresh_install(mock_paths):
    ...
    assert "nano_grive is ready" in result.stdout
...
def test_onboard_interactive_discard_does_not_save_or_create_workspace(mock_paths, monkeypatch):
    ...
    from nano_grive.cli.onboard import OnboardResult

    monkeypatch.setattr(
        "nano_grive.cli.onboard.run_onboard",
...
def test_onboard_uses_explicit_config_and_workspace_paths(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nano_grive.channels.registry.discover_all", lambda: {})
...
def test_onboard_wizard_preserves_explicit_config_in_next_steps(tmp_path, monkeypatch):
    ...
    compact_output = stripped_output.replace("\n", "")
    resolved_config = str(config_path.resolve())
    assert f'nano_grive agent -m "Hello!" --config {resolved_config}' in compact_output
    assert f"nano_grive gateway --config {resolved_config}" in compact_output
...
def test_openai_compat_provider_passes_model_through():
    ...
    with patch("nano_grive.providers.openai_compat_provider.AsyncOpenAI"):
...
def test_make_provider_uses_github_copilot_backend():
    from nano_grive.cli.commands import _make_provider
    from nano_grive.config.schema import Config
...
def test_github_copilot_provider_strips_prefixed_model_name():
    from nano_grive.providers.github_copilot_provider import GitHubCopilotProvider
...
async def test_github_copilot_provider_refreshes_client_api_key_before_chat():
    ...
    with patch("nano_grive.providers.openai_compat_provider.AsyncOpenAI", return_value=mock_client):
        provider = GitHubCopilotProvider(default_model="github-copilot/gpt-5.1")
...
def test_make_provider_passes_extra_headers_to_custom_provider():
    ...
    with patch("nano_grive.providers.openai_compat_provider.AsyncOpenAI") as mock_async_openai:
...
def mock_agent_runtime(tmp_path):
    ...
    with patch("nano_grive.config.loader.load_config", return_value=config) as mock_load_config, \
         patch("nano_grive.config.loader.resolve_config_env_vars", side_effect=lambda c: c), \
         patch("nano_grive.cli.commands.sync_workspace_templates") as mock_sync_templates, \
         patch("nano_grive.cli.commands._make_provider", return_value=object()), \
         patch("nano_grive.cli.commands._print_agent_response") as mock_print_response, \
         patch("nano_grive.bus.queue.MessageBus"), \
         patch("nano_grive.cron.service.CronService"), \
         patch("nano_grive.agent.loop.AgentLoop") as mock_agent_loop_cls:
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "nano_grive.config.loader.set_config_path",
        lambda path: seen.__setitem__("config_path", path),
    )
    monkeypatch.setattr("nano_grive.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("nano_grive.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("nano_grive.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("nano_grive.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("nano_grive.cron.service.CronService", lambda _store: object())
    ...
    monkeypatch.setattr("nano_grive.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("nano_grive.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_uses_workspace_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("nano_grive.config.loader.set_config_path", lambda _path: None)
    monkeypatch.setattr("nano_grive.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("nano_grive.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("nano_grive.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("nano_grive.bus.queue.MessageBus", lambda: object())
    ...
    monkeypatch.setattr("nano_grive.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("nano_grive.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("nano_grive.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_workspace_override_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("nano_grive.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("nano_grive.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("nano_grive.config.paths.get_cron_dir", lambda: legacy_dir)
...
def test_agent_custom_config_workspace_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("nano_grive.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr(
        "nano_grive.cli.commands._print_agent_response", lambda *_args, **_kwargs: None
    )
...
def _patch_cli_command_runtime(
    ...
    *,
    ...
    monkeypatch.setattr("nano_grive.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("nano_grive.config.loader.resolve_config_env_vars", lambda c: c)
    monkeypatch.setattr(
        "nano_grive.cli.commands.sync_workspace_templates",
        sync_templates or (lambda _path: None),
    )
    monkeypatch.setattr(
        "nano_grive.cli.commands._make_provider",
        make_provider or (lambda _config: object()),
    )

    if message_bus is not None:
        monkeypatch.setattr("nano_grive.bus.queue.MessageBus", message_bus)
    if session_manager is not None:
        monkeypatch.setattr("nano_grive.session.manager.SessionManager", session_manager)
    if cron_service is not None:
        monkeypatch.setattr("nano_grive.cron.service.CronService", cron_service)
    if get_cron_dir is not None:
        monkeypatch.setattr("nano_grive.config.paths.get_cron_dir", get_cron_dir)
...
def _patch_serve_runtime(monkeypatch, config: Config, seen: dict[str, object]) -> None:
    ...
    monkeypatch.setattr("nano_grive.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("nano_grive.api.server.create_app", _fake_create_app)
...
def test_gateway_cron_evaluator_receives_scheduled_reminder_context(
    ...
    monkeypatch.setattr("nano_grive.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("nano_grive.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("nano_grive.cli.commands._make_provider", lambda _config: provider)
    monkeypatch.setattr("nano_grive.bus.queue.MessageBus", lambda: bus)
    monkeypatch.setattr("nano_grive.session.manager.SessionManager", lambda _workspace: object())
    ...
    monkeypatch.setattr("nano_grive.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("nano_grive.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("nano_grive.channels.manager.ChannelManager", _StopAfterCronSetup)
    monkeypatch.setattr(
        "nano_grive.utils.evaluator.evaluate_response",
...
def test_migrate_cron_store_moves_legacy_file(tmp_path: Path) -> None:
    ...
    from nano_grive.cli.commands import _migrate_cron_store
    ...
    with patch("nano_grive.config.paths.get_cron_dir", return_value=legacy_dir):
