...
def test_openrouter_sets_default_attribution_headers() -> None:
    spec = find_by_name("openrouter")
    with patch("nano_grive.providers.openai_compat_provider.AsyncOpenAI") as MockClient:
    ...
    assert headers["HTTP-Referer"] == "https://github.com/JesusGon98/Nano-Grive"
    assert headers["X-OpenRouter-Title"] == "nano_grive"
...
def test_openrouter_user_headers_override_default_attribution() -> None:
    ...
    with patch("nano_grive.providers.openai_compat_provider.AsyncOpenAI") as MockClient:
        OpenAICompatProvider(
            ...
            default_model="anthropic/claude-sonnet-4-5",
            extra_headers={
                "HTTP-Referer": "https://nano_grive.ai",
    ...
    headers = MockClient.call_args.kwargs["default_headers"]
    assert headers["HTTP-Referer"] == "https://nano_grive.ai"
