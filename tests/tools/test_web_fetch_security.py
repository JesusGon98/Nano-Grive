...
from nano_grive.agent.tools.web import WebFetchTool
...
async def test_web_fetch_blocks_private_ip():
    tool = WebFetchTool()
    with patch("nano_grive.security.network.socket.getaddrinfo", _fake_resolve_private):
...
async def test_web_fetch_blocks_localhost():
    ...
    with patch("nano_grive.security.network.socket.getaddrinfo", _resolve_localhost):
...
async def test_web_fetch_result_contains_untrusted_flag():
    ...
    with patch("nano_grive.security.network.socket.getaddrinfo", _fake_resolve_public), \
...
async def test_web_fetch_blocks_private_redirect_before_returning_image(monkeypatch):
    ...
    monkeypatch.setattr("nano_grive.agent.tools.web.httpx.AsyncClient", FakeClient)
